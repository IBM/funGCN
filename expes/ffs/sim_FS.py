"""code to run the FS fungcn on synthetic data"""
...
from fungcn.ffs.generate_sim_FS import GenerateSimFS
...
if __name__ == '__main__':
    ...
    if plot:
        ...
        # for i in range(not0):
        #     if indx[i]:
        #         plt.plot(grid, out_FS.x_curves[ind_curve, :].T, '--', color='green')
        #         ind_curve += 1
        ...
        #         plt.plot(grid, out_FS.x_curves[i, :].T, '--', color='green')
        #         plt.title('true (-) vs estimated (--) x')
        #         plt.show()
